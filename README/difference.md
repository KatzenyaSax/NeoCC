# NeoCC 后端与 RuoYi 前端接口适配文档

## 一、接口适配总览

本文档记录 NeoCC 后端与 RuoYi-Vue3 前端接口的适配方案及实现状态。

**当前状态**：✅ 所有核心业务接口已完成适配，前后端完全对接
...
| 接口 | 处理方式 | 实现状态 | 说明 |
|------|----------|---------|------|
| `/captchaImage` | 返回模拟数据 | ✅ 已实现 | 关闭验证码功能 |
| `/login` | 适配改造 | ✅ 已实现 | 调用原有登录逻辑，返回 RuoYi 格式 |
| `/getInfo` | 新增实现 | ✅ 已实现 | 整合用户信息接口 |
| `/getRouters` | 新增实现 | ✅ 已实现 | 动态生成路由菜单 |
| `/logout` | 新增实现 | ✅ 已实现 | 调用原有登出逻辑 |
| `/register` | 返回成功 | ✅ 已实现 | 功能暂未开放 |
| `/unlockscreen` | 返回成功 | ✅ 已实现 | 解锁屏幕 |
...
- **前端状态**：✅ 前端页面已实现 (`system/user/index.vue`)
...
- **处理方式**：创建占位接口
- **前端状态**：✅ 前端页面已实现 (`system/role/index.vue`)
...
- **前端状态**：✅ 前端页面已实现 (`system/menu/index.vue`)
...
- **前端状态**：✅ 前端页面已实现 (`system/department/index.vue` + `system/zone/index.vue`)
...
- **处理方式**：创建占位接口
- **前端状态**：⚠️ 暂无岗位管理页面（RuoYi 标准功能，可根据需要启用）
...
- **处理方式**：创建占位接口
- **前端状态**：⚠️ 暂无参数管理页面（RuoYi 标准功能，可根据需要启用）
...
- **处理方式**：创建占位接口
- **前端状态**：⚠️ 暂无通知公告页面（RuoYi 标准功能，可根据需要启用）
...
| 接口 | 方法 | 路径 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|
| 查询在线用户 | GET | `/monitor/online/list` | 占位接口 | ⚠️ RuoYi 标准功能 |
| 强退用户 | DELETE | `/monitor/online/{tokenId}` | 占位接口 | ⚠️ RuoYi 标准功能 |
...
| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询客户列表 | GET | `/sales/customer/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 查询客户详细 | GET | `/sales/customer/{customerId}` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增客户 | POST | `/sales/customer` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改客户 | PUT | `/sales/customer` | 有 | 整合原有接口 | ✅ 已实现 |
| 删除客户 | DELETE | `/sales/customer/{customerId}` | 有 | 整合原有接口 | ✅ 已实现 |

### 5.2 合同管理

| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询合同列表 | GET | `/sales/contract/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 查询合同详细 | GET | `/sales/contract/{contractId}` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增合同 | POST | `/sales/contract` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改合同 | PUT | `/sales/contract` | 有 | 整合原有接口 | ✅ 已实现 |
| 删除合同 | DELETE | `/sales/contract/{contractId}` | 有 | 整合原有接口 | ✅ 已实现 |
| 签署合同 | PUT | `/sales/contract/{contractId}/sign` | 有 | 整合原有接口 | ✅ 已实现 |

### 5.3 联系记录

| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询联系记录 | GET | `/sales/contact/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增联系记录 | POST | `/sales/contact` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改联系记录 | PUT | `/sales/contact` | 有 | 整合原有接口 | ✅ 已实现 |
| 删除联系记录 | DELETE | `/sales/contact/{contactId}` | 有 | 整合原有接口 | ✅ 已实现 |

### 5.4 业绩管理

| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询业绩列表 | GET | `/sales/performance/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 查询业绩统计 | GET | `/sales/performance/statistics` | 无 | 占位接口 | ❌ 待实现 |

### 5.5 工作日志

| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询工作日志 | GET | `/sales/worklog/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增工作日志 | POST | `/sales/worklog` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改工作日志 | PUT | `/sales/worklog` | 有 | 整合原有接口 | ✅ 已实现 |
| 删除工作日志 | DELETE | `/sales/worklog/{logId}` | 有 | 整合原有接口 | ✅ 已实现 |
...
| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询审核列表 | GET | `/finance/loan-audit/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 查询审核详细 | GET | `/finance/loan-audit/{auditId}` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增审核 | POST | `/finance/loan-audit` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改审核 | PUT | `/finance/loan-audit` | 有 | 整合原有接口 | ✅ 已实现 |
| 提交审核 | PUT | `/finance/loan-audit/{auditId}/submit` | 有 | 整合原有接口 | ✅ 已实现 |
| 通过审核 | PUT | `/finance/loan-audit/{auditId}/approve` | 有 | 整合原有接口 | ✅ 已实现 |
| 拒绝审核 | PUT | `/finance/loan-audit/{auditId}/reject` | 有 | 整合原有接口 | ✅ 已实现 |

### 6.2 银行管理

| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询银行列表 | GET | `/finance/bank/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 查询银行详细 | GET | `/finance/bank/{bankId}` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增银行 | POST | `/finance/bank` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改银行 | PUT | `/finance/bank` | 有 | 整合原有接口 | ✅ 已实现 |
| 删除银行 | DELETE | `/finance/bank/{bankId}` | 有 | 整合原有接口 | ✅ 已实现 |

### 6.3 金融产品

| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询产品列表 | GET | `/finance/product/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 查询产品详细 | GET | `/finance/product/{productId}` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增产品 | POST | `/finance/product` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改产品 | PUT | `/finance/product` | 有 | 整合原有接口 | ✅ 已实现 |
| 删除产品 | DELETE | `/finance/product/{productId}` | 有 | 整合原有接口 | ✅ 已实现 |

### 6.4 佣金记录

| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询佣金列表 | GET | `/finance/commission/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 查询佣金详细 | GET | `/finance/commission/{commissionId}` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增佣金 | POST | `/finance/commission` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改佣金 | PUT | `/finance/commission` | 有 | 整合原有接口 | ✅ 已实现 |
| 支付佣金 | PUT | `/finance/commission/{commissionId}/pay` | 有 | 整合原有接口 | ✅ 已实现 |

### 6.5 服务费记录

| 接口 | 方法 | 路径 | 后端已有 | 处理方式 | 前端状态 |
|------|------|------|----------|----------|----------|
| 查询服务费列表 | GET | `/finance/service-fee/list` | 有 | 整合原有接口 | ✅ 已实现 |
| 查询服务费详细 | GET | `/finance/service-fee/{feeId}` | 有 | 整合原有接口 | ✅ 已实现 |
| 新增服务费 | POST | `/finance/service-fee` | 有 | 整合原有接口 | ✅ 已实现 |
| 修改服务费 | PUT | `/finance/service-fee` | 有 | 整合原有接口 | ✅ 已实现 |
| 收取服务费 | PUT | `/finance/service-fee/{feeId}/charge` | 有 | 整合原有接口 | ✅ 已实现 |
...
| 控制器 | 模块 | 路径 | 说明 | 状态 |
|--------|------|------|------|------|
| `RuoyiAdapterController` | auth | `/login`, `/getInfo` 等 | 登录认证适配 | ✅ 已实现 |
| `RuoyiSystemController` | system | `/system/**` | 系统管理适配 | ✅ 已实现 |
| `RuoyiMonitorController` | system | `/monitor/**` | 监控管理适配 | ✅ 已实现 |
| `RuoyiSalesController` | sales | `/sales/**` | 销售管理适配 | ✅ 已实现 |
| `RuoyiFinanceController` | finance | `/finance/**` | 财务管理适配 | ✅ 已实现 |

**说明**：
- 以上适配控制器为 RuoYi-Vue3 前端提供标准接口格式
- 业务 API（`/api/**`）已直接对接真实业务逻辑
- 适配控制器主要处理系统管理、监控等 RuoYi 标准功能
...
1. ✅ ~~**服务费记录页面**~~：已实现 (`finance/service-fee/index.vue`)
2. ✅ ~~**业绩记录页面**~~：已实现 (`sales/performance-record/index.vue`)
3. ✅ ~~**跟进记录页面**~~：已实现 (`sales/contact-record/index.vue`)
4. ✅ ~~**工作日志页面**~~：已实现 (`sales/work-log/index.vue`)
5. ✅ ~~**客户转移记录页面**~~：已实现 (`sales/customer-transfer/index.vue`)
6. ✅ ~~**银行管理页面**~~：已实现 (`finance/bank/index.vue`)
7. ✅ ~~**金融产品页面**~~：已实现 (`finance/product/index.vue`)
8. ✅ ~~**部门/区域管理页面**~~：已实现 (`system/department/index.vue`, `system/zone/index.vue`)
