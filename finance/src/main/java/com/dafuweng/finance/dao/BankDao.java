...
public interface BankDao extends BaseMapper<BankEntity> {
    ...
    @Select("SELECT COALESCE(MIN(t.id + 1), 1) FROM (SELECT 1 as id UNION SELECT MAX(id) + 1 FROM bank) t WHERE NOT EXISTS (SELECT 1 FROM bank b WHERE b.id = t.id AND b.deleted = 0)")
