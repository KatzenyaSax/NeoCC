...
public interface ContractDao extends BaseMapper<ContractEntity> {
    ...
    @Select("SELECT COALESCE(MIN(t.id + 1), 1) FROM (SELECT 1 as id UNION SELECT MAX(id) + 1 FROM contract) t WHERE NOT EXISTS (SELECT 1 FROM contract c WHERE c.id = t.id)")
